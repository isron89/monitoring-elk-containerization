...
for i in {1..60}; do
  if curl -s -u logstash_writer:password http://elasticsearch:9200/_cluster/health 2>/dev/null | grep -q '"status":"green"\|"status":"yellow"'; then
...
if ! curl -s -u logstash_writer:password http://elasticsearch:9200/ 2>/dev/null | grep -q "cluster_name"; then
